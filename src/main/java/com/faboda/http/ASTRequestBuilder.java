...
public class ASTRequestBuilder {
  ...
  private void setHeaders() {
    ...
    headers =
        headerNodes.stream()
            .collect(Collectors.toMap(ASTNode::getKey, ASTNode::getValue, (r1, r2) -> r1));
  ...
  private void setUrl() {
    String url =
        astNode.getChildren().stream()
            .filter(node -> node.getType().equals(NodeType.URL))
            .findFirst()
            .get()
            .getValue();
  ...
  private void setData() {
    ...
    String encoding =
        isApplicationJsonEncoded()
            ...
            : MediaTypes.APPLICATION_X_WWW_FORM_URLENCODED.getMediaTypeValue();
    Map<String, String> data =
        dataNodes.stream().collect(Collectors.toMap(ASTNode::getKey, ASTNode::getValue));
  ...
  private boolean isApplicationJsonEncoded() {
    List<String> headerKeys =
        astNode.getChildren().stream()
            .filter(nd -> nd.getType().equals(NodeType.HEADER))
